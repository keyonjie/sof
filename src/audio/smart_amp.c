...
static void free_mem_load(struct smart_amp_data *cd)
{
	if (!cd) {
		comp_cl_err(&comp_smart_amp, "free_mem_load(): invalid cd");
		return;
	}
				 ...
				 uint32_t frames, int8_t *chan_map)
{
	...
	comp_dbg(dev, "smart_amp_process_s32(), out_chan:%d", sad->out_channels);
